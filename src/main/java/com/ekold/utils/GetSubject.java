...
public class GetSubject {
    ...
    //卡券详情导出头
    public static String[] couponDetailExpHeads = {"题干", "选项A", "选项B", "选项C", "选项D"};

    static String url = "http://47.96.77.18/ks/ksxunlian1.php?km=%E9%A3%8E%E9%99%A9%E5%90%88%E8%A7%84%E8%80%83%E8%AF%95%E9%A2" +
            "%98%E5%BA%93[saadmin]&lx=%E5%8D%95%E9%80%89&mm=xPVE13TXpVM05gxNjAwMjZ8MTUzNzk1NDM1N3N3NxbFtzcF00MqVXdbc3BdODc" +
            "=jExODIxOTkwMD";
    ...
    public static void buildExcel() throws Exception {
        FileOutputStream out = null;
        out = new FileOutputStream("d://题目.xls");
        HSSFWorkbook workbook = getHSSFWorkbook("题目", couponDetailExpHeads, sendGet(), null);
        workbook.write(out);
    }

    public static void buildText(List<SubjectVO> voList) throws Exception {
        String path = "d:\\单选.txt";
        ...
        for (SubjectVO subjectVO : voList) {
            buffer.append(i);
            buffer.append(".");
            buffer.append(subjectVO.getSubject()).append("\r\n").append(subjectVO.getOptionA()).append
                    ("\r\n").append(subjectVO.getOptionB()).append("\r\n").append(subjectVO.getOptionC()).append("\r\n").append
                    (subjectVO.getOptionD());
            buffer.append("\r\n");
            buffer.append("\r\n");
    ...
    public static List<SubjectVO> sendGet() throws Exception {
        ...
        for (int i = 0; i < 30; i++) {
            ...
            for (Element element : subjectAll) {
                ...
                options = element.html().toString().split("<br>");
                vo.setSubject(element.select("B").get(0).html());
                vo.setOptionA(options[2]);
                vo.setOptionB(options[3]);
                vo.setOptionC(options[4]);
                vo.setOptionD(options[5]);
